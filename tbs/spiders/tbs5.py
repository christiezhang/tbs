...
class tbs5(BaseSpider):
    ...
    def __init__(self):
        ...
        self.start_urls=['http://mzy.100ppi.com/price/list---1.html']
    ...
    def parse(self,response):
          ...
          maxissuestimestamp = self.getmaxtimestamp()
          for x in range(2,31):
            ...
            issuestimestamp = int(time.mktime(time.strptime(issuestime, "%Y-%m-%d")))
            if issuestimestamp > maxissuestimestamp:
                insertsql = 'INSERT INTO `mzy_price` ( `id` , `name` , `style` , `price` , `unit` , `spec` , `origin` , `issuestime` , `issuestimestamp`, `source` , `crawldate` )\
                         VALUES (\
                         NULL , \''+mname+'\', \''+style+'\', \''+price+'\', \''+unit+'\', \''+spec+'\', \''+orgin+'\', \''+issuestime+'\',\''+str(issuestimestamp)+'\'' \
    ...
    def getmaxtimestamp(self):
        sql = 'select max(issuestimestamp) AS issuestimestamp from mzy_price'
        try:
              self.databaseobject.cursor.execute(sql)
              rows =  self.databaseobject.cursor.fetchall()
              return rows[0][0]
        except Exception as e:
               print e
               return 0
